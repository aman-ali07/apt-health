...
detect_broken_deps() {
  local output
  if [ "$EUID" -ne 0 ]; then
    log_warn "Broken dependency check requires root; run with sudo for full results."
    return 0
  fi
...
detect_locked_apt() {
  ...
  if [ "$EUID" -ne 0 ]; then
    log_warn "APT lock check requires root; run with sudo for full results."
    return 0
  fi
  ...
  for lock in "${locks[@]}"; do
    if [ -e "$lock" ]; then
      if command_exists fuser; then
        ...
        found=1
        log_warn "APT lock file present (holder unknown): $lock"
      fi
    fi
  done
